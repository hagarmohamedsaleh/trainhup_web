...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $logo = ''; // Initialize the variable to store the logo path
    ...
    // Update data in the "company_requirements" table using a prepared statement to prevent SQL injection
    // Use placeholders in the prepared statement
    $sql = $conn->prepare("UPDATE company_requirement SET description=?, phone=?, logo=? WHERE name=?");
    ...
    $sql->bind_param("ssss", $description, $phone, $logo, $name);

    if ($sql->execute()) {
       // echo "Company information updated successfully";
    } else {
        echo "Error updating company information: " . $sql->error;
    }
...
<main>
    <h1 data-lang="edit_profile">Edit profile</h1>
    <br><br>
    <div class="edit-profile-page">
        <form action="" class="edit-profile" enctype="multipart/form-data" method="post">
            <div class="edit-profile-content">
                <label for="" data-lang="edit_image">Edit image profile</label>
                <input type="file" placeholder="Enter your name" name="logo">
            </div>
            <div class="edit-profile-content">
                <label for="" data-lang="edit_name">Display name</label>
                <input type="text" placeholder="Enter your name" name="name">
            </div>

            <div class="edit-profile-content">
                <label for="" data-lang="Desc">Description</label>
                <input type="text" placeholder="Enter Description" name="descripton">
            </div>

            <div class="edit-profile-content">
                <label for="" data-lang="edit_phone">Phone</label>
                <input type="number" placeholder="Enter your phone number" name="phone">
            </div>

            <button data-lang="update" type="submit" data-lang="update">update</button>
        </form>
    </div>
